<x-app-layout>
    <div class="container-fluid">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h2 class="h3 mb-0 text-dark fw-bold">Editar Colaborador</h2>
            ...
            <form action="{{ route('hr.employees.update', $employee) }}" method="POST">
                ...
                <div class="row g-4">
                    <div class="col-md-12">
                        ...
                        <input type="text" name="name" id="name" class="form-control form-control-lg bg-light border-0 @error('name') is-invalid @enderror" value="{{ old('name', $employee->name) }}" required autofocus>
                        @error('name')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>

                    <div class="col-md-6">
                        <label for="document" class="form-label fw-bold text-muted small">Documento (CPF/RG) *</label>
                        <input type="text" name="document" id="document" class="form-control bg-light border-0 @error('document') is-invalid @enderror" value="{{ old('document', $employee->document) }}" required>
                        @error('document')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>

                    <div class="col-md-6">
                        <label for="position" class="form-label fw-bold text-muted small">Cargo / Função</label>
                        <input type="text" name="position" id="position" class="form-control bg-light border-0 @error('position') is-invalid @enderror" value="{{ old('position', $employee->position) }}" placeholder="Ex: Secretário, Zelador, Diretor">
                        @error('position')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>

                    <div class="col-md-6">
                        <label for="hire_date" class="form-label fw-bold text-muted small">Data de Contratação</label>
                        <input type="date" name="hire_date" id="hire_date" class="form-control bg-light border-0 @error('hire_date') is-invalid @enderror" value="{{ old('hire_date', $employee->hire_date ? $employee->hire_date->format('Y-m-d') : '') }}">
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="is_active" id="is_active" class="form-select bg-light border-0 @error('is_active') is-invalid @enderror">
                            <option value="1" {{ old('is_active', $employee->is_active) == '1' ? 'selected' : '' }}>Ativo</option>
                            <option value="0" {{ old('is_active', $employee->is_active) == '0' ? 'selected' : '' }}>Inativo</option>
                        </select>
                ...
                <div class="text-end">
                    <button type="submit" class="btn btn-primary fw-bold shadow-sm px-4">
                        <i class="bi bi-check-lg me-2"></i> Atualizar Colaborador
                    </button>
                </div>
            </form>
        </div>
